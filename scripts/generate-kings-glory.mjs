 ...
 *
 * 用法：
 *   node scripts/generate-kings-glory.mjs --list
 *   node scripts/generate-kings-glory.mjs all --dry-run
 *   node scripts/generate-kings-glory.mjs boss simp thin-k rebel joker drunk dior-s food-ie
 *   node scripts/generate-kings-glory.mjs all
 *   node scripts/generate-kings-glory.mjs all --force
 */
...
import { KINGS_TYPES, buildKingsPrompt } from './kings-glory-prompts.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const OUTPUT_DIR = path.join(__dirname, '../public/images/types/kings');
...
async function generateOne(config, type, force) {
  ...
  const prompt = buildKingsPrompt(type);
...
async function main() {
  ...
  if (listMode) {
    console.log(KINGS_TYPES.map((type) => `${type.slug} (${type.heroName} / ${type.heroRef})`).join('\n'));
    return;
  }
  ...
  const selected =
    slugArgs.length === 0 || slugArgs.includes('all')
      ? KINGS_TYPES
      : KINGS_TYPES.filter((type) => slugArgs.includes(type.slug));

  if (selected.length === 0) {
    throw new Error(`No matching slugs. Available: ${KINGS_TYPES.map((type) => type.slug).join(', ')}`);
  }
