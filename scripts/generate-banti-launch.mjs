 ...
 *
 * 用法：
 *   node scripts/generate-banti-launch.mjs --list
 *   node scripts/generate-banti-launch.mjs all --dry-run
 *   node scripts/generate-banti-launch.mjs boss mum party malo
 *   node scripts/generate-banti-launch.mjs all --force
 */
...
import { BANTI_LAUNCH_TYPES, buildBantiLaunchPrompt } from './banti-launch-prompts.mjs';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const OUTPUT_DIR = path.join(__dirname, '../public/images/types/banti');
...
async function generateOne(config, type, force) {
  ...
  const prompt = buildBantiLaunchPrompt(type);

  console.log(`\n🎨 [${type.slug}] ${type.workName}`);
...
async function main() {
  ...
  if (listMode) {
    console.log(BANTI_LAUNCH_TYPES.map((type) => type.slug).join(', '));
    return;
  }
  ...
  const selected =
    slugArgs.length === 0 || slugArgs.includes('all')
      ? BANTI_LAUNCH_TYPES
      : BANTI_LAUNCH_TYPES.filter((type) => slugArgs.includes(type.slug));

  if (selected.length === 0) {
    throw new Error(`No matching slugs. Available: ${BANTI_LAUNCH_TYPES.map((type) => type.slug).join(', ')}`);
  }
  ...
  if (dryRun) {
    ...
    selected.forEach((type) => {
      const prompt = buildBantiLaunchPrompt(type);
      console.log(`[${type.slug}] ${type.workName}`);
