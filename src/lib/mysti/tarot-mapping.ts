...
export const MYSTI_TAROT_MAP: Record<string, MystiTarotData> = {
  ...
  party: {
    ...
    tagline: '在狂欢中，你触碰自由',
    reading: '你是最会「活在当下」的人。每一次聚会你都是气氛担当，每一次冒险你都冲在最前面。但当音乐停下、灯光熄灭，你是否敢面对安静的自己？试着在不嗨的时候也和自己待一会儿。',    shadowReading: '高塔在你的宇宙里是「宗醜」的别名。狂欢的能量透支后，等待你的不是休息，而是坠落。你怕的不是萃废，是安静中那个空洞洞的自己。',
  ...
  joker: {
    ...
    reading: '你用幽默化解一切——尴尬、痛苦、沉重。这让身边的人很舒服，但也让你的真实感受永远没人看见。你的课题是学会在某些时刻摘下面具，让别人看到你不笑的样子，也会被爱。',    shadowReading: '你的高塔是笑话讲不动的那一刻。当幽默失效、解构崩塔，你会发现自己从来没有练习过「不好笑的时候该怎么办」。那是你最脆弱的时刻，也是最真实的。',
  ...
  drunk: {
    ...
    tagline: '放纵是另一种冲锋',
    reading: '你追求极致的释放——喝酒、冒险、做决定不过脑子。这些冲动背后是一种对「活着」的强烈渴望。但无节制的自由最终会反噬你。学会把这股能量引导到有建设性的方向，你会所向披靡。',    shadowReading: '你的高塔是宿醜后的那个早晨。酒醒之后看到的废墟——说错的话、伤害的人、做过的舂事——会让你意识到，自由和毁灭之间只隔了一杯酒。',
  ...
  woc: {
    ...
    shadowReading: '倒吊人是你尖叫之后的沉默。当所有人都被你的怒火吓跑之后，倒吊人会问：你到底是在生气，还是在求救？没有观众的愤怒，最终只会反噮自己。',
  ...
  malo: {
    ...
    shadowReading: '你的恶魔不是欲望，而是麻木。当「不在乎」变成「什么都感受不到」，躺平就从清醒变成了慈缓死亡。你以为你在休息，其实你在熄灭。',
  ...
  sleep: {
    ...
    reading: '睡眠是你的庇护所。在梦里，你比醒着的时候更自由、更真实。你对休息的需求不是懒惰，而是一种深度自我修复。但别让庇护所变成监狱——醒来之后的世界也有值得你探索的风景。',    shadowReading: '月亮在你的梦境和现实之间画了一条越来越模糊的线。当睡眠不再是休息而是逃难，你的梦境就变成了另一个现实的替代品。你不是在修复，你是在藏身。',
