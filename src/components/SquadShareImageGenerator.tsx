...
async function createQrImage() {
  const url = await QRCode.toDataURL(SHARE_SITE_URL, {
    width: 200, margin: 1,
        ...
        {previewUrl && (
          ...
          >
            ...
            >
              <button
                onClick={() => setPreviewUrl(null)}
                className="absolute -top-11 -right-1 p-2 text-white/50 hover:text-white transition-colors z-10 cursor-pointer"
                aria-label="关闭"
              >
