...
export function CciPanel({ variant = 'light', className = '' }: Props) {
  ...
  const wrap = isDark
    ? 'rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm text-bg-primary'
    : 'rounded-2xl border border-border-subtle bg-bg-secondary text-text-primary';

  if (profiles.length < 2) {
    return (
      <div className={`${wrap} px-5 py-4 ${className}`}>
        <p className={`text-[11px] tracking-[0.2em] uppercase ${isDark ? 'text-bg-primary/55' : 'text-text-muted'}`}>
          CCI · 跨情境一致性
        </p>
        <p className={`mt-2 text-[13px] leading-relaxed ${isDark ? 'text-bg-primary/75' : 'text-text-secondary'}`}>
  ...
  return (
    <div className={`${wrap} px-5 py-4 ${className}`}>
      <div className="flex items-baseline justify-between">
        ...
        <p className={`text-[10px] ${isDark ? 'text-bg-primary/45' : 'text-text-muted'}`}>
          基于 {result.universeCount} 个宇宙
        </p>
      </div>
      <div className="mt-2 flex items-baseline gap-3">
        <span className="text-4xl font-semibold tracking-tight">{result.total}</span>
        <span className={`text-sm ${isDark ? 'text-bg-primary/70' : 'text-text-secondary'}`}>
          {result.bandLabel}
        </span>
      </div>
      <p className={`mt-1 text-[12px] leading-relaxed ${isDark ? 'text-bg-primary/65' : 'text-text-secondary'}`}>
        {result.bandTagline}
      </p>
        ...
        {(['W', 'T', 'F', 'I'] as const).map(a => {
          ...
          return (
            ...
            >
              ...
              <div className={`text-base font-semibold ${isDark ? 'text-bg-primary' : 'text-text-primary'}`}>
                {score}
              </div>
            </div>
          );
