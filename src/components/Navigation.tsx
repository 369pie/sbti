...
const NAV_ITEMS = [
  ...
  { href: '/jueti/', label: '🪞 觉TI' },
  { href: '/xpti/', label: '💜 恋爱XP' },
  { href: '/wtfti/work/', label: '💼 班TI' },
        ...
        <div className="flex items-center gap-1 sm:gap-2 overflow-x-auto scrollbar-hide">
          {NAV_ITEMS.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="px-3 py-1.5 rounded-lg text-[13px] sm:text-sm whitespace-nowrap transition-all duration-200 shrink-0 text-text-muted hover:text-text-secondary hover:bg-bg-secondary"
            >
              {item.label}
            </Link>
          ))}
