...
async function renderDrunkShareImage(persona: DrunkPersonaType, dimensionScores: DrunkDimensionScore[]) {
  ...
  // Card border
  strokeRoundedRect(ctx, 14, 14, CARD_WIDTH - 28, CARD_HEIGHT - 28, 24, hexToRgba(persona.color, 0.4), 2.5);
  strokeRoundedRect(ctx, 22, 22, CARD_WIDTH - 44, CARD_HEIGHT - 44, 18, hexToRgba(persona.color, 0.1), 1);

  // Corner ornaments
  ctx.fillStyle = hexToRgba(persona.color, 0.45);
  ...
  if (charImage) {
    ...
    ctx.fillText(persona.emoji, CARD_WIDTH / 2, imgY + 70);
  }
  ...
  ctx.textAlign = 'center';
  ctx.fillText(`「${persona.tagline}」`, CARD_WIDTH / 2, tagY + 14);
  ctx.textAlign = 'left';
