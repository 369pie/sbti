...
const DRUNK_SHARE_URL = SHARE_SITE_URL + 'drunk/';
...
/** Extract a short description — first ~4 meaningful sentences */
...
async function renderDrunkShareImage(persona: DrunkPersonaType, dimensionScores: DrunkDimensionScore[]) {
  const BG = '#FAF8F5';
  const DARK = '#1F1A16';
  const MED = '#5B524B';
  const LIGHT = '#9A908A';
  ...
  wash.addColorStop(0, hexToRgba(persona.color, 0.09));
  wash.addColorStop(1, hexToRgba(persona.color, 0));
  ctx.fillStyle = wash;
  ...
  ctx.textAlign = 'center';
  ctx.fillText('酒后人设鉴定', CARD_WIDTH / 2, y);
  y += 24;
  ctx.textAlign = 'left';
  ...
  if (charImage) {
    ...
    ctx.textAlign = 'center';
    ctx.fillText(persona.emoji, CARD_WIDTH / 2, y + 70);
    ctx.textAlign = 'left';
  }
  y += imgH + 14;
  ...
  fillRoundedRect(ctx, 36, y, quoteW, quoteH, 16, hexToRgba(persona.color, 0.06));
  strokeRoundedRect(ctx, 36, y, quoteW, quoteH, 16, hexToRgba(persona.color, 0.15));
  ctx.fillStyle = persona.color;
  ctx.font = `600 17px ${FONT_SANS}`;
  ctx.textAlign = 'center';
  ...
  // ============ ★ DESCRIPTION ============
  drawDivider(ctx, y, hexToRgba(persona.color, 0.3));
  y += 16;
  ...
  const shortDesc = extractShortDesc(persona.description);
  ctx.fillStyle = DARK;
  ...
  ctx.font = `11px ${FONT_MONO}`;
  ctx.textAlign = 'center';
  ctx.fillText('五维数据', CARD_WIDTH / 2, y);
  y += 20;

  dimensionScores.forEach((score) => {
    const dim = DRUNK_DIMENSIONS.find(d => d.id === score.id);
    if (!dim) return;
    const color = DRUNK_MODEL_COLORS[dim.model].base;
  ...
  drawDivider(ctx, y, hexToRgba(persona.color, 0.2));
  y += 14;

  ctx.fillStyle = MED;
  ctx.font = `italic 12px ${FONT_SANS}`;
  ctx.textAlign = 'center';
  ctx.fillText(persona.tagline, CARD_WIDTH / 2, y);
  y += 28;
  ctx.textAlign = 'left';
  ...
  ctx.textAlign = 'left';
  ctx.fillText('测测你的酒后人设？', 36, footerY + 12);

  ctx.fillStyle = persona.color;
  ctx.font = `11px ${FONT_MONO}`;
  ctx.fillText(DRUNK_SHARE_URL, 36, footerY + 34);
  ...
  strokeRoundedRect(cctx, 14, 14, CARD_WIDTH - 28, CARD_HEIGHT - 28, 24, hexToRgba(persona.color, 0.4), 2.5);
  strokeRoundedRect(cctx, 22, 22, CARD_WIDTH - 44, CARD_HEIGHT - 44, 18, hexToRgba(persona.color, 0.1), 1);

  // Corner ornaments
  cctx.fillStyle = hexToRgba(persona.color, 0.45);
...
export const DrunkShareImageGenerator = forwardRef<DrunkShareImageGeneratorHandle, Props>(
  function DrunkShareImageGenerator({ persona, dimensionScores }, ref) {
      ...
      if (isMobile()) {
        try {
          ...
          if (file && navigator.share && navigator.canShare?.({ files: [file] })) {
            ...
            await navigator.share({ files: [file], title: `DRUNK-${persona.code}.png` });
            return;
          }
        } catch (error) {
    ...
    const handleShare = useCallback(async () => {
      ...
      try {
        const file = await createPreviewFile();
        if (file && navigator.share && navigator.canShare?.({ files: [file] })) {
          await navigator.share({ files: [file], title: `我的酒后人设：${persona.name}` });
        } else {
          ...
          >
            ...
            >
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={handleShare}
                  className="flex-1 py-3 rounded-xl bg-amber-500 text-white text-sm font-medium hover:brightness-110 transition-all cursor-pointer flex items-center justify-center gap-2"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
