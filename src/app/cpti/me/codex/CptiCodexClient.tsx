  ...
  return (
    <div className="min-h-screen bg-[#F5F0E8] text-[#2c2620]">
      <div className="mx-auto max-w-[820px] px-5 py-10 sm:py-14">
        ...
        <p className="mt-3 max-w-[520px] text-[14px] leading-[1.85] text-[#6b5b3f]">
        ...
        {(records.length >= 3 || upgradeIntent) && (
          <div className="mt-6 rounded-2xl border border-[#c9a676]/50 bg-white/80 p-4 text-[13px] leading-[1.7] text-[#5a4f3e] sm:flex sm:items-center sm:gap-4">
            <div className="flex-1">
              <p className="font-medium text-[#2c2620]">
                {records.length >= 3 ? '把所有关系永久留下来' : '先看年卡会把这本图鉴变成什么'}
              </p>
              <p className="mt-1 text-[12px] text-[#8a7355]">
                {records.length >= 3
            ...
            <Link
              ...
              className="mt-3 inline-flex items-center justify-center rounded-lg bg-[#2c2620] px-4 py-2 text-[12.5px] font-medium text-[#F5F0E8] sm:mt-0 sm:shrink-0"
            >
        ...
        {notifications.length > 0 && (
          <div className="mt-6 rounded-xl border border-[#C9A676] bg-[#FFF8E8] p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 space-y-1">
                <p className="text-[11px] tracking-[0.3em] uppercase text-[#8a6d3a]">Invite Loopback · {notifications.length}</p>
                <p className="text-sm text-[#2c2620]">
                  你分享过的 {notifications.length} 段关系，已经有人点开并看完了。
                </p>
                <ul className="text-xs text-[#5b4b36] space-y-0.5 pt-1">
                  {notifications.slice(0, 3).map(n => (
                    <li key={n.id}>
                      · 有人打开了你分享的「{getRelationshipBySlug(n.relationshipSlug)?.name ?? n.relationshipSlug}」
                    </li>
                  ))}
                  {notifications.length > 3 && <li className="text-[#8a6d3a]">…还有 {notifications.length - 3} 条</li>}
                </ul>
              </div>
              <button
                onClick={dismissNotifications}
                className="shrink-0 text-[11px] text-[#8a6d3a] hover:text-[#2c2620]"
              >
                全部标记已读
              </button>
            </div>
  ...
  return (
    <button
      ...
      className={`rounded-full border px-3.5 py-1.5 text-[12.5px] transition ${
        active
          ? 'border-[#2c2620] bg-[#2c2620] text-[#F5F0E8]'
          : 'border-[#c9a676]/40 bg-white/70 text-[#5a4f3e] hover:border-[#c9a676]'
      }`}
    >
      {children}
    </button>
...
function EmptyState({ tab }: { tab: Tab }) {
  ...
  return (
    <div className="rounded-2xl border border-dashed border-[#c9a676]/50 bg-white/70 px-6 py-12 text-center">
      <p className="text-[14px] text-[#6b5b3f]">你的{label}图鉴还是空白。</p>
      <Link
        href="/cpti/test/"
        className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#2c2620] px-4 py-2.5 text-[13px] font-medium text-[#F5F0E8]"
      >
        先去测一段 →
      </Link>
    </div>
...
function RecordCard({ record, editing, onEditStart, onEditEnd, onUpdate, onDelete }: RecordCardProps) {
  ...
  return (
    <article
      className="rounded-2xl border border-[#e8d5dc] bg-white/92 p-4 shadow-[0_8px_24px_-18px_rgba(192,122,142,0.35)]"
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline gap-2">
            <h3 className="truncate text-[15px] font-medium text-[#2c2620]">{rel.name}</h3>
            <span
            ...
            {record.compatibility !== undefined && (
              <span className="text-[11px] font-mono text-[#8a7355]">
        ...
        <div className="flex shrink-0 gap-1">
          <Link
            ...
            className="rounded-md border border-[#c9a676]/40 px-2 py-1 text-[11px] text-[#6b5b3f] hover:bg-[#c9a676]/10"
          >
            查看
          </Link>
          <button
      ...
      {(record.partnerNickname || record.note) && !editing && (
        <div className="mt-3 border-t border-[#e8d5dc] pt-3 text-[13px] leading-[1.7]">
          {record.partnerNickname && (
            <p className="text-[#3a342a]">
              <span className="text-[#8a7355]">昵称：</span>{record.partnerNickname}
            </p>
          )}
          {record.note && (
            <p className="mt-1 text-[#5a4f3e]">
              <span className="text-[#8a7355]">备注：</span>{record.note}
            </p>
          )}
        </div>
      )}
      ...
      {editing && (
        <div className="mt-3 space-y-3 border-t border-[#e8d5dc] pt-3">
          <label className="block text-[12px] text-[#6b5b3f]">
            ta 的昵称
            <input
              value={nickname}
              ...
              className="mt-1 block w-full rounded-md border border-[#c9a676]/40 bg-white px-3 py-2 text-[13px] text-[#2c2620] outline-none focus:border-[#c9a676]"
            />
          </label>
          <label className="block text-[12px] text-[#6b5b3f]">
            备注
          ...
          <div className="flex justify-between gap-2">
            <button
              onClick={onDelete}
              className="rounded-md px-3 py-1.5 text-[12px] text-[#a04646] hover:bg-[#fff0f0]"
            >
              删除
            </button>
            <div className="flex gap-2">
              <button
                onClick={onEditEnd}
                className="rounded-md border border-[#c9a676]/40 px-3 py-1.5 text-[12px] text-[#6b5b3f]"
              >
                取消
              </button>
              <button
                onClick={() => { onUpdate({ partnerNickname: nickname.trim(), note: note.trim(), scenario }); onEditEnd(); }}
                className="rounded-md bg-[#2c2620] px-3 py-1.5 text-[12px] text-[#F5F0E8]"
              >
                保存
              </button>
            </div>
