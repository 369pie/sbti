  ...
  return (
    <div className="min-h-screen" style={{ background: '#0a0a0a' }}>
      ...
      >
        ...
        {personality.number && (
          <p className="text-xs text-white/30 tracking-widest mb-2">{personality.number}</p>
        )}
        <h1 className="text-3xl font-bold text-white mb-2">{personality.name}</h1>
        {personality.code && (
          <p className="text-sm text-white/40 font-mono mb-3">{personality.code}</p>
        )}
        {personality.tagline && (
          <p className="text-white/60 text-sm leading-relaxed">{personality.tagline}</p>
        )}
      </motion.section>
      ...
      <div className="max-w-lg mx-auto px-6 space-y-8 pb-12">
        ...
        {personality.copy_hit && (
          ...
          >
            <h2 className="atelier-section-header text-sm font-medium text-white/60 mb-3 flex items-center gap-2">
              <Glyph name={'strike' as GlyphName} size={15} tone="gold" />
              <span className="tracking-wide">
                {stripLeadingEmoji(universe.hit_label) || '一击'}
              </span>
            </h2>
            <p className="text-white/80 text-sm leading-relaxed">{personality.copy_hit}</p>
          </motion.section>
        )}
        ...
        {personality.quote && (
          ...
          >
            <p className="text-white/30 text-xs italic">&ldquo;{personality.quote}&rdquo;</p>
          </motion.div>
        )}
        ...
        >
          <button
            onClick={handleShare}
            className="px-8 py-3 rounded-full text-white text-sm font-medium transition-transform active:scale-95"
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <Link href={`/creator/profile/${creator.id}/`} className="text-sm font-medium text-white hover:text-white/80 transition-colors">
                    {creator.name}
                  </Link>
                  {creator.is_verified && <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-500/15 text-blue-300">已认证</span>}
                </div>
                <div className="text-xs text-white/30 mt-1">本测试创作者</div>
                {creator.bio && (
                  <div className="text-sm text-white/55 mt-2 leading-7 line-clamp-3">{creator.bio}</div>
                )}
                <div className="flex flex-wrap gap-3 mt-3">
                  <Link
                    href={`/creator/profile/${creator.id}/`}
                    className="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/15 text-xs text-white/80 transition-colors"
                  >
                    看 TA 的其他宇宙
                  </Link>
                  {creator.social_link && (
                    <a
                      ...
                      className="px-3 py-1.5 rounded-lg border border-white/10 hover:bg-white/5 text-xs text-white/55 transition-colors"
                    >
                      外部主页 ↗
                    </a>
                  )}
        ...
        {otherPersonalities.length > 0 && (
          <section className="pt-8">
            <h3 className="text-sm text-white/30 mb-4 text-center">其他人格类型</h3>
            <div className="grid grid-cols-2 gap-3">
              {otherPersonalities.map(p => (
                ...
                >
                  ...
                  {p.tagline && (
                    <div className="text-xs text-white/40 mt-1 line-clamp-2">{p.tagline}</div>
                  )}
                </a>
              ))}
            </div>
