...
export function LoveResultContent({ personality, dimensionScores }: Props) {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `我的恋爱人设是 ${personality.code}（${personality.name}）`,
  ...
  const others = LOVE_PERSONALITY_TYPES.filter(p => p.slug !== personality.slug).slice(0, 3);
        ...
        >
          ...
          <div className="rounded-2xl border border-border-subtle bg-bg-elevated shadow-sm p-6 sm:p-8 space-y-5">
            {dimensionScores.map(ds => {
              const dim = LOVE_DIMENSIONS.find(d => d.id === ds.id);
              if (!dim) return null;
              const color = LOVE_MODEL_COLORS[dim.model];
                ...
                <div key={ds.id}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-text-primary">{LOVE_MODEL_NAMES[dim.model]}</span>
                    </div>
        ...
        >
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={quickShare}
                className="flex-1 py-3 rounded-xl border border-pink-500/30 text-sm text-pink-400 hover:bg-pink-500/10 transition-all cursor-pointer flex items-center justify-center gap-2"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
                </svg>
